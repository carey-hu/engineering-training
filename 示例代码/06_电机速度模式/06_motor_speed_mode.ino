/*
  ...
  串口中输入的是输出轴转速，单位 RPM（转/分钟），更直观易懂。
  ...
  串口命令：
  数字  设置输出轴目标转速 RPM，如 30 表示每分钟转 30 圈
  +/-   加减 2 RPM
  t     停止
  s     读取状态
  e     清除错误
...
#define MAX_OUTPUT_RPM  60.0f  // 最大输出轴转速限制 RPM
#define SPEED_STEP    2.0f   // 转速调整步长 RPM
...
#define CMD_CLEAR_ERROR  0x9B  // 清除错误命令

HardwareSerial RS485(2);  // 使用UART2
float targetOutputRpm = 0.0f;  // 输出轴目标转速 RPM
unsigned long lastReadTime = 0;  // 上次读取状态时间戳
uint8_t readFailCount = 0;  // 状态读取失败计数
...
void setSpeed(float outputRpm) {  // 设置输出轴转速（RPM）
  outputRpm = constrain(outputRpm, -MAX_OUTPUT_RPM, MAX_OUTPUT_RPM);  // 限制转速范围
  targetOutputRpm = outputRpm;

  float outputDps = outputRpm * RPM_TO_DPS;  // 转换为 dps
  ...
  Serial.printf(">>> 输出轴目标转速: %.1f RPM (%.1f dps)，协议原始值: %ld\n",
                targetOutputRpm, outputDps, (long)raw);
}
...
void readStatus() {  // 读取电机状态
  ...
  if (!readFrame(CMD_READ_STATE2, data, &len) || len < 7) {  // 接收响应帧
    readFailCount++;  // 累计失败次数
    if (readFailCount <= 3) {  // 只显示前3次失败
      Serial.println("状态读取失败。");
    } else if (readFailCount == 4) {
      Serial.println("状态读取持续失败，请检查接线和电源。");
    }
    return;
  }

  readFailCount = 0;  // 成功后重置计数
  ...
  float outputDps = motorDps / REDUCTION;  // 计算输出轴速度 dps
  float outputRpm = outputDps / RPM_TO_DPS;  // 转换为 RPM

  Serial.printf("温度: %d C, 电流采样: %d, 输出轴: %.1f RPM (%.1f dps), 编码器: %u\n",
                temperature, iq, outputRpm, outputDps, encoder);
}

void clearError() {  // 清除电机错误
  sendCommand(CMD_CLEAR_ERROR);
  uint8_t data[16], len = 0;
  if (readFrame(CMD_CLEAR_ERROR, data, &len)) {
    Serial.println(">>> 错误已清除");
  } else {
    Serial.println("清除错误失败");
  }
}
...
void handleCommand() {  // 处理串口命令
  ...
  if (input == "t") {  // 停止命令
    ...
    setSpeed(targetOutputRpm + SPEED_STEP);
  } else if (input == "-") {  // 减速命令
    setSpeed(targetOutputRpm - SPEED_STEP);
  } else if (input == "s") {  // 查询状态命令
    readStatus();
  } else if (input == "e") {  // 清除错误命令
    clearError();
...
void printHelp() {  // 打印帮助信息
  Serial.println("\n===== MG4010-i10 速度闭环控制 =====\n"
                 "数字  设置输出轴转速 RPM，例如 30（每分钟30圈）\n"
                 "+/-   加减 2 RPM\n"
                 "t     停止\n"
                 "s     读取状态\n"
                 "e     清除错误\n"
                 "h     帮助\n"
                 "建议首次测试从 5 到 10 RPM 开始。\n"
                 "==================================\n");
}
...
void loop() {
  ...
  if (targetOutputRpm != 0 && millis() - lastReadTime > 500) {  // 运行时每500ms读取状态
    lastReadTime = millis();
    readStatus();
  }
}
