...
> 本指南帮助你从零开始理解舵机，避开常见坑，最终实现网页控制舵机。
...
舵机部分建议作为机器人机构控制模块来学习，不需要按固定时间切分。建议按下面顺序推进：

| 阶段 | 目标 | 建议产出 |
|-----|------|---------|
| 认识舵机 | 理解 5V 供电、GND 共地、PWM 信号线 | 能正确画出单舵机接线图 |
| 单舵机基础控制 | 用固定角度验证舵机方向和活动范围 | 舵机能转到 0°、90°、180° |
| 串口调试 | 通过串口输入目标角度 | 能用串口快速验证机构动作 |
| 非阻塞控制 | 用 `millis()` 平滑更新角度 | 舵机动作时主循环不卡顿 |
| 多舵机扩展 | 将一个舵机的写法扩展到四个舵机 | 四个舵机能分别设定角度 |
| 工程整合 | 与两个 RS485 电机放入同一主程序 | 参考 `示例代码/10_机器人整合控制/` |

先完成“能稳定控制一个舵机”，再扩展到四个舵机。不要在供电、共地和角度范围还没验证清楚时直接装到机器人结构上。
...
2. **非阻塞编程思想**
   ...
   - `millis()` 计时原理
   - 状态机思想
   - 多舵机和电机同时运行时，所有周期性动作都应放在主循环中调度

3. **调试技巧**
   - 用串口输出调试信息
   - 用万用表测量电压
   - 逐步排查问题
   - 先脱开机械负载测试，再安装到机器人结构上
...
**问题 2**：代码中用了 `delay()`，导致 WiFi 卡顿

**现象**：网页控制舵机时，点击按钮没反应

**解决**：对比阻塞和非阻塞写法，理解两种写法对主循环的影响

---

**问题 3**：把舵机接到 ESP32 的 3.3V 引脚
