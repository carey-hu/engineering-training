...
本课程不是固定学时的单次实验，而是面向机器人制作的项目制训练。建议按“先能用、再理解、最后整合”的顺序推进：

1. **开发板认识与环境验证**
   学生需要知道 USB 口、EN/BOOT 键、GPIO、3.3V、5V、GND 的含义，完成 Arduino IDE、ESP32-S3 板卡包、串口驱动和 LED 闪烁验证。

2. **基础调试能力训练**
   通过串口输出、串口输入和简单网页控制，让学生掌握 `Serial.println()`、串口监视器、Wi-Fi AP 和浏览器访问 ESP32 的基本方法。

3. **PWM 舵机模块学习**
   从单个舵机开始，理解 50Hz PWM、脉宽与角度的关系、外部 5V 供电和共地要求，再扩展到多个舵机的非阻塞控制。

4. **RS485 电机模块学习**
   先完成单个 MG4010-i10 电机的运行、停止、清错、状态读取，再学习速度模式、位置模式和网页控制。重点理解厂家私有协议、ID、校验和、减速比换算和安全上电顺序。

5. **机器人底盘与机构整合**
   将两个 RS485 电机作为左右轮或双执行机构，将四个 PWM 舵机作为机器人机构关节，统一放入一个主循环中调度。最终工程参考 `示例代码/10_机器人整合控制/` 和 `教程文档/09_robot_integration.md`。

6. **项目迭代与验收**
   学生在能稳定控制硬件后，再加入传感器、结构件、运动逻辑和任务流程。验收重点不只看“能动”，还要看接线安全、程序结构、调试记录和异常处理。
